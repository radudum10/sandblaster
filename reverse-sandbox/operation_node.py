...
def build_operation_node_graph(node, default_node):
    ...
    # If node is non-terminal and has already been processed, then it's a jump
    # rule to a previous operation.
    ...
    while nodes_to_process:
        ...
        if default_node.terminal.is_deny():
            # In case of non-terminal match and deny as unmatch, add match to
            # path.
            if current_node.non_terminal.is_non_terminal_deny():
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
            # In case of non-terminal match and allow as unmatch, do a not
            # (reverse), end match path and add unmatch to parent path.
            ...
            # In case of non-terminals, add match to path and unmatch to parent
            # path.
            elif current_node.non_terminal.is_non_terminal_non_terminal():
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
                ong_add_to_parent_path(g, current_node, parent_node, nodes_to_process)
            # In case of allow as match and non-terminal unmatch, end path and
            # add unmatch to parent path.
            elif current_node.non_terminal.is_allow_non_terminal():
                ong_end_path(g, current_node, parent_node, nodes_to_process)
                ong_add_to_parent_path(g, current_node, parent_node, nodes_to_process)
            # In case of deny as match and non-terminal unmatch, do a not
            # (reverse), and add match to path.
            elif current_node.non_terminal.is_deny_non_terminal():
                ong_mark_not(g, current_node, parent_node, nodes_to_process)
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
            # In case of deny as match and allow as unmatch, do a not
            # (reverse), and end match path (completely).
            elif current_node.non_terminal.is_deny_allow():
                ong_mark_not(g, current_node, parent_node, nodes_to_process)
                ong_end_path(g, current_node, parent_node, nodes_to_process)
            # In case of allow as match and deny as unmatch, end match path
            # (completely).
            elif current_node.non_terminal.is_allow_deny():
                ong_end_path(g, current_node, parent_node, nodes_to_process)
        elif default_node.terminal.is_allow():
            # In case of non-terminal match and deny as unmatch, do a not
            # (reverse), end match path and add unmatch to parent path.
            if current_node.non_terminal.is_non_terminal_deny():
                ong_mark_not(g, current_node, parent_node, nodes_to_process)
                ong_end_path(g, current_node, parent_node, nodes_to_process)
                ong_add_to_parent_path(g, current_node, parent_node, nodes_to_process)
            # In case of non-terminal match and allow as unmatch, add match to
            # path.
            elif current_node.non_terminal.is_non_terminal_allow():
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
            # In case of non-terminals, add match to path and unmatch to parent
            # path.
                ...
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
                ong_add_to_parent_path(g, current_node, parent_node, nodes_to_process)
            # In case of allow as match and non-terminal unmatch, do a not
            # (reverse), and add match to path.
            elif current_node.non_terminal.is_allow_non_terminal():
                ong_mark_not(g, current_node, parent_node, nodes_to_process)
                ong_add_to_path(g, current_node, parent_node, nodes_to_process)
            # In case of deny as match and non-terminal unmatch, end path and
            # add unmatch to parent path.
            elif current_node.non_terminal.is_deny_non_terminal():
                ong_end_path(g, current_node, parent_node, nodes_to_process)
                ong_add_to_parent_path(g, current_node, parent_node, nodes_to_process)
            # In case of deny as match and allow as unmatch, end match path
            # (completely).
            elif current_node.non_terminal.is_deny_allow():
                ong_end_path(g, current_node, parent_node, nodes_to_process)
            # In case of allow as match and deny as unmatch, do a not
            # (reverse), and end match path (completely).
            elif current_node.non_terminal.is_allow_deny():
                ong_mark_not(g, current_node, parent_node, nodes_to_process)
                ong_end_path(g, current_node, parent_node, nodes_to_process)
...
def _remove_duplicate_node_edges(g, node, start_list):
    ...
    for n in nexts:
        if n in start_list:
            ...
            if n not in nodes_traversed_for_removal:
...
class ReducedGraph:
    ...
    def reduce_graph_with_metanodes(self):
        ...
        # Add require-all metanode if current node has a require-any as a
        # predecessor and is followed by another node.
        copy_vertices = list(self.vertices)
        for v in copy_vertices:
    ...
    def reduce_integrated_vertices(self, integrated_vertices):
        ...
        while True:
            if not re.search("entitlement-value #t", str(v)):
                require_all_vertices.append(v)
            next_vertices = self.get_next_vertices(v)
            if decision is None and v.decision is not None:
                decision = v.decision
            self.remove_vertice(v)
            if v in self.final_vertices:
                self.final_vertices.remove(v)
